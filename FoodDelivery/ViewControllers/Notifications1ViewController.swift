...
class Notifications1ViewController: DSViewController {
    ...
    func turnOnNotificationSection() -> DSSection {
        ...
        composer.add(type: .body,
                     text: "Get new offers first, track your delivery.")
    ...
    func shipButton() -> DSViewModel {
        ...
        let button = DSButtonVM(title: "Another time", type: .cleanWithBorder) { [unowned self] _ in
            self.dismiss()
        }
...
struct Notifications1ViewControllerPreview: PreviewProvider {
    ...
    static var previews: some View {
        Group {
            PreviewContainer(VC: Notifications1ViewController(), OrangeAppearance()).edgesIgnoringSafeArea(.all)
        }
    }
}
