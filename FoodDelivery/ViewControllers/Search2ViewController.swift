...
open class Search2ViewController: DSViewController {
    ...
    func update() {
        ...
        var label = DSLabelVM(.headline, text: "Searching", alignment: .center)
        var button = DSButtonVM(title: "Demo", icon: UIImage(systemName: "timer")) { btn in
            print("Tap on button")
        }
        button.width = .absolute(100)
        button.height = .absolute(40)
        
        
        label.supplementaryItems = [button.asSupplementary(position: .rightCenter)]
        showTop(content: label.list())
...
struct Search2ViewControllerPreview: PreviewProvider {
    ...
    static var previews: some View {
        Group {
            PreviewContainer(VC: Search2ViewController()).edgesIgnoringSafeArea(.all)
        }
    }
}
